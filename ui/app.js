...
// Первый кадр из снимка прошлого визита, синхронно, не дожидаясь сервера.
// Иначе страница рождается пустой и на глазах наполняется: сетка вскакивает,
// счётчики перещёлкиваются с нулей. Данные могли устареть, но через мгновение
// их поправит настоящий ответ, а одинаковые экран не перерисовывает вовсе.
try {
  const m = JSON.parse(sessionStorage.getItem('tanba-triage') || 'null');
  if (m && m.inbox) { state = m; render(); }
