...
// Первый кадр из снимка прошлого визита. Без него страница рождалась
// в состоянии по умолчанию, и это было видно глазами: тумблер автозапуска
// стоял выключенным и через мгновение сам перещёлкивался во включённый.
// Живой ответ сервера придёт следом и молча поправит, что изменилось.
try {
  const m = JSON.parse(sessionStorage.getItem('tanba-settings') || 'null');
  if (m && m.update) { s = m; render(); }
} catch (e) { /* снимка нет */ }

addEventListener('pagehide', () => {
  try { if (s) sessionStorage.setItem('tanba-settings', JSON.stringify(s)); } catch (e) { }
});
...
// До первого ответа сервера рисовать нечего: render читает снимок s,
// а сверка языка могла обогнать его загрузку.
I18N.sync(() => { paintLang(); if (s) render(); });
paintTheme();
paintLang();
