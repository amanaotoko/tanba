...
public sealed class ПотеряДанных
{
    ...
    /// <summary>
    /// Опасная ветка того же переноса: обе папки существуют разом, и в обеих
    /// лежит файл с одним именем. Это не выдумка, так и вышло на живом
    /// хранилище: Corel пишет резервные копии под одним именем, и две таких
    /// оказались в разных папках, с разным содержимым.
    ///
    /// Затереть нельзя, оставить тоже: оставленный файл держал бы старую
    /// папку на диске навсегда. Значит переносим под соседним именем.
    /// </summary>
    [Fact]
    public void Переименование_приёма_сливает_две_папки_и_не_затирает_тёзок()
    {
        ...
        // Обе копии на месте, ни одна не затёрта, и старой папки больше нет.
        Assert.Equal("новая копия", File.ReadAllText(Path.Combine(b.Cfg.Inbox, "тёзка.cdr")));
        Assert.Equal("старая копия", File.ReadAllText(Path.Combine(b.Cfg.Inbox, "тёзка (2).cdr")));
        Assert.False(Directory.Exists(Path.Combine(b.Cfg.Root, Config.OldInboxName)));
    }
