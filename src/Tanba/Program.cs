...
// а обращение к хранилищу там ещё и упало бы, потому что его может не быть.
//
// Обратных вызова два. При удалении убираем за собой запись автозапуска:
// пишем её мы, и убрать её больше некому, см. Startup.Forget. А после
// обновления помечаем, что нас перезапустили: ключи запуска новой копии
// выбирает старая, и та, что стоит у человека сейчас, поднимает нас в трей.
// Человек в этот момент смотрит в экран настроек и ждёт окна, а не значка.
var restarted = false;
VelopackApp.Build()
    .OnRestarted(_ => restarted = true)
    .OnBeforeUninstallFastCallback(_ => Startup.Forget())
    .Run();
...
// Дешёвая проверка на каждом запуске: обновление портит тип одного значения
// в записи об установленной программе, и починить его один раз нельзя.
Registration.Fix();

var toTray = args.Contains(Args.Tray) && !restarted;
...
// Копии, поставленные до того, как выбор начали запоминать, лечим на месте:
// иначе мастер открывался бы у них и дальше, при каждом запуске. Заодно это
// запоминает и путь, пришедший из TANBA_ROOT: переменная всё равно старше
// файла и в следующий раз снова победит, а без неё программа теперь знает,
// куда идти, вместо того чтобы спрашивать.
if (RootStore.Read() is null) RootStore.Write(cfg.Root);
