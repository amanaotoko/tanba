...
public static class Args
{
    ...
    /// Открыть мастер настройки, даже если место уже выбрано и запомнено.
    /// С этим ключом программа перезапускает сама себя по кнопке
    /// «Настроить заново» на экране настроек.
    public const string Setup = "--setup";

    /// Подождать выхода указанного процесса, прежде чем занимать порт и
    /// мьютекс. Нужно при перезапуске самого себя: прошлая копия ещё жива
    /// в тот момент, когда новая уже стартовала.
    public const string WaitPid = "--waitpid";
}
